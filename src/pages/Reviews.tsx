...
interface VoteCounts { [reviewId: string]: { likes: number; dislikes: number } }
interface MyVotes { [reviewId: string]: "like" | "dislike" }
...
export default function Reviews() {
  ...
  // Reconcile a list with a fresh server snapshot. Server is source of truth:
  // it adds new rows, updates changed ones, and removes deleted ones.
  const reconcile = <T extends { id: string }>(_prev: T[], next: T[]): T[] => next;
  ...
  const fullSync = useCallback(async () => {
    ...
    if (reviewsRes.data) setReviews((prev) => reconcile(prev, (reviewsRes.data as any[]).map(stripReview)));
    if (repliesRes.data) setReplies((prev) => reconcile(prev, (repliesRes.data as any[]).map(stripReply)));
  }, []);
  ...
  useEffect(() => {
    ...
    // Realtime subscription for reviews — incremental merge (no full refetch).
    // RLS may block anon clients from receiving review payloads; the 5-min
    // fallback poll below catches anything missed.
    const reviewsChannel = supabase
      .channel('reviews-incremental-realtime')
    ...
    // Realtime subscription for replies — incremental merge.
    const repliesChannel = supabase
      .channel('review-replies-incremental-realtime')
