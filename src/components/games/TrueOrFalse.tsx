  ...
  return (
    <div className="flex flex-col items-center gap-4 md:gap-6 w-full max-w-lg mx-auto">
      <div className="flex items-center justify-between w-full">
        ...
        <span className="text-xs md:text-sm text-muted-foreground">Score: {score}/{questions.length}</span>
      </div>
      ...
      <Card className="w-full border-2">
        <CardContent className="p-4 md:p-6 text-center space-y-3">
          <p className="text-xs uppercase tracking-widest text-muted-foreground">Begrip</p>
          <h2 className="text-xl md:text-2xl font-bold text-primary">{current.term}</h2>
          <div className="border-t pt-3">
            <p className="text-xs uppercase tracking-widest text-muted-foreground mb-2">Omschrijving</p>
            <p className="text-sm md:text-base">{current.shownDefinition}</p>
          </div>
        </CardContent>
      </Card>

      <p className="text-sm text-muted-foreground font-medium">Klopt deze omschrijving bij dit begrip?</p>
        ...
        <div className="w-full space-y-3">
          <Card className={answered ? "border-[hsl(var(--success))] bg-[hsl(var(--success))]/5" : "border-destructive bg-destructive/5"}>
            <CardContent className="p-4 text-center">
              {answered ? (
                <p className="font-semibold text-[hsl(var(--success))]">✅ Goed!</p>
              ) : (
                <div>
                  <p className="font-semibold text-destructive mb-1">❌ Fout!</p>
                  <p className="text-sm text-muted-foreground">
                    De juiste omschrijving: <span className="font-medium text-foreground">{current.correctDefinition}</span>
                  </p>
                </div>
              )}
            </CardContent>
